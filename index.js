...
//     cb(null, uploadsDir); // Save all files in the `uploads/images` directory
//   },
...
//   } catch (err) {
//     res.status(500).json({ message: "Error saving content", error: err.message });
//   }
// });
...
// MongoDB Connection
mongoose
  .connect("mongodb+srv://manish:manish@cluster0.ulkk9.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")
  .then(() => console.log("MongoDB Connected"))
  .catch((err) => console.error("MongoDB Connection Error:", err));
...
const ContentSchema = new mongoose.Schema({
  ...
  paragraphs: [String],
  imageUrl: { type: String, required: true },
  readMoreLink: { type: String, default: "" }, // New field for the read more link
});
const Content = mongoose.model("Content", ContentSchema);
...
// Save or update content
app.post("/content", async (req, res) => {
  const { headerText, descriptionText, paragraphs, imageUrl, readMoreLink } = req.body;
  ...
  try {
    ...
    if (content) {
      ...
      content.paragraphs = paragraphs;
      content.imageUrl = imageUrl;
      content.readMoreLink = readMoreLink;  // Save the new read more link
      await content.save();
    } else {
      // Create new content
      content = new Content({
        ...
        paragraphs,
        imageUrl,
        readMoreLink,  // Save the new read more link
      });
      await content.save();
    }
    res.json({ success: true, content });
